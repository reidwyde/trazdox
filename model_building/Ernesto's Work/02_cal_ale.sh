...
for g in $(seq 1 ${groups}); do
    ...
    if [ ${g} -eq 1 ]; then
        ...
        echo -n "plot [0:36][0:] " | tee -a figura.cmd &> /dev/null
        sed -n -e "1,6p" mean_std_group_${g}.dat > tmp.txt
        echo -n "'tmp.txt' u 1:2:3 t'' with yerrorbars lw 2 pt ${point_type[${i}]} ${point}," | tee -a figura.cmd &> /dev/null
        echo -n "'tmp.txt' u 1:2 t'Data' w lp lw 2 pt ${point_type[${i}]} ${point}," | tee -a figura.cmd &> /dev/null
        sed -n -e "1,6p" tumor_evolution_${g}.txt > tmp2.txt
        echo -n "'tmp2.txt' u 1:2 t'Model' w lp lw 2 pt ${point_type[${i}]} ps 0.75 lc 7" | tee -a figura.cmd &> /dev/null
    fi
    gnuplot "figura.cmd"
    ...
    if [ ${g} -eq 1 ]; then
        ...
        sed -n -e "1,6p" error_g${g}p.txt > tmp.txt
...
for l in $(seq 1 ${NL}); do
    ...
    echo "\centering"
    echo "\includegraphics[width=\textwidth]{ale/solution_group_${l}}\caption{\$${mean}\pm ${stdv}\%\$}"
    echo "\end{subfigure}"
done
echo "\end{figure}"
cp *.pdf ../New_Models/figures/ale/
#cd ../New_Models/
#./lista.sh
